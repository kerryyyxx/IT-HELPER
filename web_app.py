        ...
        if choice == "💡 灵感对话助手":
            ...
            if pr := st.chat_input("输入你的疑问..."):
                ...
                with st.chat_message("assistant"):
                    ...
                    # --- 1. 设置 Gemini 风格的系统指令 ---
                    sys_prompt = (
    "你是BitBite，一位资深的信息技术与编程导师。你的风格是‘启发式教学’，目标是帮助高中生透彻理解概念。\n"
    "行为准则：\n"
    "1. 【精准解惑】：当学生问概念（如：AI是什么）时，严禁背诵百科。请用生活中的类比（如：把AI比作人的大脑学习经验）来解释，确保高中生能听懂。\n"
    "2. 【循循善诱】：不要一次性灌输所有知识。先解释核心逻辑，然后给出一个启发性的观察点，引导学生思考这个概念在现实中（或编程中）的意义。\n"
    "3. 【尊重意愿】：除非学生主动提出想做项目（如：我想写个程序），否则不要强行要求学生动手。只针对他当前的问题进行深度且有趣的拆解。\n"
    "4. 【语言风格】：语气温和、客观、专业。严禁使用‘复读机’式的长篇大论，保持对话的自然感。\n"
    "5. 【禁止跨界】：仅回答与信息技术、计算机科学、编程相关的问题。若涉及其他领域，礼貌地引导回技术讨论。"
                    )
                    ...
                    try:
                        stream = client.chat.completions.create(
                            model="THUDM/GLM-4-9B-0414",  # 替换为硅基流动的免费模型
